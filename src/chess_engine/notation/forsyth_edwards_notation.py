...
class FenChars:
    ...
    def get_piece_fen(char: str, is_white_turn: bool) -> str:
        if char not in FenChars.get_possible_pieces():
            raise Exception("char not a piece value")
        if is_white_turn:
            return char.upper()
        return char.lower()
...
class Fen:
    ...
    def get_castling_rights(self, from_piece_val: str, from_index: int) -> str:
        if self.data.castling_rights == FenChars.EMPTY_INFO:
            return self.data.castling_rights
    ...
    def get_en_passant_rights(self, from_piece_val: str, from_index: int, dest_index: int) -> str:
        ...
        if from_piece_val != pawn_fen:
            return FenChars.EMPTY_INFO
        if from_index not in double_moves_start:
            return FenChars.EMPTY_INFO
        if dest_index not in double_moves_end:
            return FenChars.EMPTY_INFO
    ...
    def get_half_move_clock(self, from_piece_val: str, dest_piece_val: str) -> str:
        pawn_fen = FenChars.get_piece_fen(FenChars.DEFAULT_PAWN, self.is_white_turn())
        if from_piece_val == pawn_fen or dest_piece_val != FenChars.BLANK_PIECE:
            return '0'
    ...
    def get_pieces_count(self) -> dict[str, int]:
        ...
        for piece_fen in iterate(self.data.piece_placement):
            if piece_fen == FenChars.BLANK_PIECE:
                continue
            if piece_fen.isnumeric():
                continue
    ...
    def is_move_en_passant(self, from_index: int, dest_index: int) -> bool:
        pawn_fen = FenChars.get_piece_fen(FenChars.DEFAULT_PAWN, self.is_white_turn())
        if self.data.en_passant_rights == '-':
            return False
        if self[from_index] != pawn_fen:
            return False
    ...
    def is_move_castle(self, from_index: int, dest_index: int) -> bool:
        ...
        if self.data.castling_rights == FenChars.EMPTY_INFO:
            return False
        if self[from_index] != king_fen:
            return False
        if self[dest_index] != rook_fen:
            return False
        return True
...
def validate_fen_piece_placement(piece_placement) -> bool:
    ...
    for piece_fen in iterate(piece_placement):
        if piece_fen.isnumeric():
            ...
            if int_fen == 0:
                raise Exception('INVALID fen notation: number too small')
            if int_fen > 8:
                raise Exception('INVALID fen notation: number too big')
            count += int_fen
        else:
...
def validate_fen_val(fen_val: str, expanded_values: bool = False) -> bool:
    ...
    if expanded_values:
        possible_values.append(FenChars.BLANK_PIECE)
...
def validate_fen_castling_rights(castling_rights: str) -> bool:
    ...
    if len(castling_rights) > 4:
        raise Exception('too many castling rights chars')
    if len(castling_rights) == 0:
        raise Exception('castling rights cannot be empty')
...
def validate_fen_en_passant_rights(en_passant_rights: str) -> bool:
    if len(en_passant_rights) > 2:
        raise Exception('too many digits')
    if en_passant_rights != FenChars.EMPTY_INFO:
        validate_file_and_rank(*en_passant_rights)
    return True


def validate_fen_half_move_clock(half_move_clock: str) -> bool:
    if not half_move_clock.isnumeric():
        raise Exception('half move clock cannot be a letter')
    if int(half_move_clock) < 0:
        raise Exception('half move clock cannot be negative')
    if int(half_move_clock) > 100:
        raise Exception('half move clock cannot be over 100')
    return True


def validate_fen_full_move_number(full_move_number: str) -> bool:
    if not full_move_number.isnumeric():
        raise Exception('full move number cannot be a letter')
    if int(full_move_number) < 0:
        raise Exception('full move number cannot be negative')
    return True
...
def decode_fen_notation(fen_notation: str) -> FenData:
    ...
    if len(data) != 6:
        raise Exception("number of arguments in fen notation is incorrect")
