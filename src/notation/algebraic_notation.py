...
class AlgebraicNotation:
    ...
    def get_an_from_index(index: int) -> AlgebraicNotation:
        ...
        if index > 63:
            raise IndexError("index cannot be above 63")
...
def validate_file_and_rank(file: str, rank: str) -> bool:
    if len(file) != 1 or len(rank) != 1:
        raise Exception("cannot be empty or larger than 1 char")
    if file.isnumeric():
        raise Exception("col should be letter")
    if not rank.isnumeric():
        raise Exception("row should be number")
    if int(rank) > BOARD_SIZE or file > 'h':
