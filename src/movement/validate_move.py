...
def is_move_valid(from_index: int, dest_index: int, fen: Fen) -> bool:
    if not is_from_valid(fen, from_index):
        return False
    if not is_side_valid(from_index, dest_index, fen):
        return False
    if not is_destination_valid(from_index, dest_index, fen):
        return False
    return True
...
def get_all_available_moves(fen: Fen, is_white_turn: None | bool = None, *, own_moves: bool) -> list[int]:
    ...
    for index, fen_char in enumerate(fen.expanded):
        ...
        if fen_char == FenChars.BLANK_PIECE:
            continue
        if not same_side if own_moves else same_side:
            continue
...
def is_stale_mate(fen: Fen) -> bool:
    if is_check(fen, not fen.is_white_turn()) or is_check(fen, fen.is_white_turn()):
        return False
...
def is_from_valid(fen: Fen, from_index: int) -> bool:
    ...
    if from_fen_val == FenChars.BLANK_PIECE:
        return False
    if not is_from_correct_side(from_fen_val, fen.is_white_turn()):
        return False
    return True


def is_side_valid(from_index: int, dest_index: int, fen: Fen) -> bool:
    if fen.is_move_castle(from_index, dest_index):
        return True
    if from_index == dest_index:
        return False
    if is_same_team(fen[from_index], fen[dest_index]):
        return False
    return True
...
def is_destination_valid(from_index: int, dest_index: int, fen: Fen) -> bool:
    ...
    if dest_index not in available_moves:
        return False
    return True
